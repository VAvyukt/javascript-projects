...
function preload() {
    ...
    groundSensor.width = 28;
    groundSensor.height = 40;
    groundSensor.layer = 9;
    groundSensor.collider = "none";
    groundSensor.visible = false;

    // ######## SIGN #########
    sign = new Sprite();
    signImg = loadImage("https://codehs.com/uploads/c2b3d3a350687e35a0df6cb7d07064e4");
    sign.img = signImg;
    sign.scale = 2;
    sign.offset.y = -25;
    sign.width = 10;
...
function draw() {
    ...
    //Character jumps
    groundSensor.moveTowards(character.x, character.y + 25, 1);
    if(groundSensor.overlapping(grassGroup)) {
        if(kb.presses("up") || kb.pressing("up")) {
            character.vel.y = -9;
        }
    }

    // Loop through every coin in the group
    for (let coin of coinGroup) {
        // Code that you will add to check for character and coin overlap
        if(character.overlaps(coin)) {
            coin.remove();
    ...
    score.x = camera.x + 180;

    if(character.y > 2*height) {
        character.speed = 0;
        character.x = startingX;
        character.y = startingY;
    }

    if(character.overlapping(sign)) {
        if(kb.presses("down")) {
            gameOver();
        }
    }
...
function setupLevel1() {
    ...
    groundSensor.x = startingX;
    groundSensor.y = startingY + 25;
