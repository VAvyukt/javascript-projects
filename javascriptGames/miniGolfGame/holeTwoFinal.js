...
let coordinates;
let grass, hole, ball, putter, lossHole;
let music;
let score;
let hits = 0;

function preload() {
    music = loadSound('https://codehs.com/uploads/fa0b0c54c3e892dd9034c112e74767c2');
}
...
function draw() {
    ...
    //Moves putter if mouse is dragging.
    ...
    //Rotates putter if r key is pressed.
    if(kb.presses('r')) {
        putter.rotate(45, 10);
    }
    
    //Slows down the ball when hit due to friction.
    if(ball.speed > 0) {
        ball.speed = ball.speed - 0.01;
    }
    
    //Removes the ball once overlaps with hole.
    //Pseudocode: if(ball overlaps the hole {then remove the ball});
    if(ball.overlaps(hole)) {
        ...
        score.width = 230;
        if (hits == 1) {
            score.text = 'You Won! [' + hits + ' hit]';
        }
        else {
            score.text = 'You Won! [' + hits + ' hits]';
        }
        score.color = '#72d62b';
        
    }
    
    if(ball.overlaps(lossHole)) {
        ball.remove();
        putter.remove();
        music.stop();
        score.width = 230;
        if (hits == 1) {
            score.text = 'You Lost! [' + hits + ' hit]';
        }
        else {
            score.text = 'You Lost! [' + hits + ' hits]';
        }
        score.color = '#cf281f';
    }
    
    if(putter.collides(ball)) {
        hits++;
...
function setupHole() {
    ...
    let midObst = new Sprite();
    midObst.collider = 'kinematic';
    midObst.color = '#e3d10e';
    midObst.width = 160;
    midObst.height = 25;
    midObst.rotationSpeed = 50;
    ...
    lossHole = new Sprite();
    lossHole.diameter = 15;
    lossHole.color = '#d4330f';
    lossHole.x = hole.x;
    lossHole.y = 120;
    lossHole.collider = 'static';
}
